...
{
	/* 0 or 1 */
    uint8_t PEIE:1; 	/* parity error */
    uint8_t TXEIE:1;	/* Transmit data register empty */
    uint8_t TCIE:1;     /* Transmission complete */
    uint8_t RXNEIE:1;   /* Read data register not empty */
    uint8_t IDLEIE:1;   /* IDLE line detected */
    uint8_t CTSIE:1;    /* CTS  interrupt */
...
//uint8_t USART_u8SendData(USART_Config_t *pUSARTHandle, uint8_t *pTxBuffer, uint32_t Length);
